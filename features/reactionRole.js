...
async function roleAdd (client, reaction, user) {
	...
	if (roles) member.roles.add(roles).catch((error) => console.error(`Something went wrong when adding a reaction role: ${error}`));
}
...
async function roleRemove (client, reaction, user) {
	...
	if (roles) member.roles.remove(roles).catch((error) => console.error(`Something went wrong when removing a reaction role: ${error}`));
}
