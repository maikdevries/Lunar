...
async function roleAdd (client, reaction, user) {
	...
	if (!roles.every((role) => rolePositionCheck(client, reaction.message.guild, role))) return console.error(`Client role lower than role when adding reaction role for guild: ${reaction.message.guild.id}`);
...
async function roleRemove (client, reaction, user) {
	...
	if (!roles.every((role) => rolePositionCheck(client, reaction.message.guild, role))) return console.error(`Client role lower than role when removing reaction role for guild: ${reaction.message.guild.id}`);
