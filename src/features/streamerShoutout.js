...
async function execute (client, newPresence) {
	...
	if (!guildSettings.enabled || !await checkGuildPermissions(client, newPresence.guild, [Permissions.FLAGS.MANAGE_ROLES]) || !checkRolePosition(client, newPresence.guild, guildSettings.shoutoutRole)) return;
	...
	if (newPresence.activities.some((activity) => activity.type === 'STREAMING')) return newPresence.member?.roles.add(guildSettings.shoutoutRole);
